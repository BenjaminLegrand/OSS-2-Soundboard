...
class MaterialDialogComponent @Inject constructor(private val baseActivity: BaseActivity) : DialogComponent {
    ...
    override fun displaySingleChoiceDialog(title: Int, choices: List<String>, selected: Int, positiveText: Int, onPositiveClick: (Int) -> Unit, negativeText: Int, onNegativeClick: () -> Unit) {
        materialDialog?.dismiss()
        materialDialog = MaterialDialog(baseActivity).show {
            title(title)
            listItemsSingleChoice(initialSelection = selected, items = choices, waitForPositiveButton = true) { _, index, _ ->
